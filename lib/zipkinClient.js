    ...
    if (!local_opts.localTesting) {
      ...
      scribeClient.open(function (err){
        if (err) console.log("Error connecting to scribe. No tracing enabled.", err);
          ...
          var tracer = new nodeTracers.ZipkinTracer(scribeClient,local_opts.scribeStoreName,{'maxTraces':local_opts.maxTraces});
          local_opts.zipkinTracers.push(tracer);
          tracers.pushTracer(tracer);
        }
      });
    }
    for (var i=0; i<local_opts.zipkinTracers.length; i++){
