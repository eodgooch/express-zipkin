...
(function () {
  ...
  var traceFilter = function(req, res, next){
    ...
    if (t){
      ...
      t.record(trace.Annotation.serverRecv(moment().millisecond() * 1000));
      req.zipkinTrace = t;
    }
    ...
    res.on('finish', function(){
      if (req.zipkinTrace) {
        ...
        t.record(trace.Annotation.serverSend(moment().millisecond() * 1000));
      }
      next();
    });
  ...
  var traceService = function(req, res, serviceName, fn, next){
    if (req.zipkinTrace) {
      ...
      t.record(trace.Annotation.serverSend(Date.now() * 1000));
      try{
        fn();
      } finally {
        t.record(trace.Annotation.serverRecv(Date.now() * 1000));
        next();
      }
    }
  };
